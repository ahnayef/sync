        ...
        <section className="glass relative overflow-hidden rounded-2xl sm:rounded-3xl border border-[var(--color-border)] p-4 sm:p-5 lg:p-6 shadow-[0_24px_60px_rgba(0,0,0,0.16)]">
          <div className="relative flex flex-col gap-3 sm:gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div className="space-y-1 sm:space-y-2">
              <h1 className="text-xl sm:text-2xl md:text-[24px] lg:text-[26px] font-bold text-[var(--color-text-primary)]">
                Teacher Management
              </h1>
              <p className="text-xs sm:text-sm text-[var(--color-text-secondary)]">
                Manage teachers, their short codes, and departments.
              </p>
            </div>
        ...
        <section className="rounded-2xl sm:rounded-3xl border border-[var(--color-border)] bg-[var(--color-bg-surface)] p-3 sm:p-4 lg:p-5 shadow-[0_18px_48px_rgba(0,0,0,0.12)]">
          <div className="flex flex-col gap-2 sm:gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="relative w-full lg:max-w-[520px]">
              <FiSearch
                className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 text-[var(--color-text-muted)]"
                size={16}
              />
              <input
                id="teacher-search"
                ...
                className="w-full rounded-lg sm:rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-elevated)] py-2 sm:py-3 pr-9 sm:pr-12 pl-9 sm:pl-11 text-xs sm:text-sm text-[var(--color-text-primary)] outline-none placeholder:text-[var(--color-text-muted)]"
              />
              {search && (
                <button
                  onClick={() => setSearch("")}
                  title="Clear search"
                  className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 rounded-lg p-1.5 text-[var(--color-text-muted)] transition-colors hover:bg-[var(--color-bg-surface)] hover:text-[var(--color-text-primary)]"
                >
                  <FiX size={14} />
                </button>
              )}
              ...
              <thead>
                <tr className="bg-[var(--color-bg-elevated)] border-b border-[var(--color-border)]">
                  {["Name", "Short Form", "Department", "Actions"].map((h) => (
                    <th
                      key={h}
                      className="px-3 sm:px-4 py-2.5 sm:py-3 text-left text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {filtered.map((t, i) => (
                  ...
                  >
                    <td className="px-3 sm:px-4 py-2.5 sm:py-3">
                      <div className="flex items-center gap-2 sm:gap-3">
                        <div
                          className={`${getAvatarClass(t.id)} w-7 sm:w-8 h-7 sm:h-8 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0`}
                        >
                          {t.name?.charAt(0) || "?"}
                        </div>
                    ...
                    <td className="px-3 sm:px-4 py-2.5 sm:py-3">
                      <code className="text-[10px] sm:text-xs font-semibold text-[var(--color-accent)] bg-[var(--color-accent-muted)] px-2 py-0.5 rounded">
                        {t.short}
                      </code>
                    </td>
                    <td className="px-3 sm:px-4 py-2.5 sm:py-3">
                      <div className="text-xs sm:text-sm text-[var(--color-text-primary)]">
                        {departmentById.get(t.department_id)?.name ||
                          t.dept ||
                          "—"}
                      </div>
                    </td>
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td
                      colSpan={4}
                      className="px-3 sm:px-4 py-8 text-center text-xs sm:text-sm text-[var(--color-text-secondary)]"
                    >
                      No teachers found.
                    </td>
                  </tr>
            ...
            {filtered.map((t) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`${getAvatarClass(t.id)} w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white flex-shrink-0`}
                  >
                    {t.name?.charAt(0) || "?"}
                  </div>

                  <div className="min-w-0 flex-1">
                    <p className="text-sm sm:text-base font-semibold text-[var(--color-text-primary)] truncate">
                      {t.name}
                    </p>
                    <p className="text-xs text-[var(--color-text-muted)] truncate">
                      {departmentById.get(t.department_id)?.name || t.dept || "—"}
                    </p>
                  </div>

                  <div className="flex flex-col items-end gap-2">
                    <code className="text-[11px] font-semibold text-[var(--color-accent)] bg-[var(--color-accent-muted)] px-2 py-0.5 rounded">
                      {t.short}
                    </code>
                    <div className="flex items-center gap-2">
                      <button
                        id={`teacher-edit-mobile-${t.id}`}
                        onClick={() => openEdit(t)}
            ...
            <div className="w-full max-w-2xl rounded-lg sm:rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-surface)] p-4 sm:p-5 lg:p-6 my-4 sm:my-8 max-h-[90vh] overflow-y-auto">
              <div className="flex items-start justify-between gap-3 sm:gap-4 border-b border-[var(--color-border)] pb-3 sm:pb-4">
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-[var(--color-text-secondary)]">
                    Provide full name, short form, and select department.
                  </p>
                </div>
                <button
                  onClick={() => setShowModal(false)}
                  className="rounded-lg p-2 text-[var(--color-text-muted)] transition-colors hover:bg-[var(--color-bg-elevated)] hover:text-[var(--color-text-primary)] flex-shrink-0"
              ...
              <div className="flex flex-col gap-3 sm:gap-4 pt-3 sm:pt-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-elevated)] py-2 sm:py-2.5 px-3 text-xs sm:text-sm text-[var(--color-text-primary)] outline-none"
                  />
                </div>
                ...
                <div className="flex gap-2 sm:gap-3 mt-2 sm:mt-4">
                  <button
                    id="modal-cancel"
                    onClick={() => setShowModal(false)}
                    className="flex-1 rounded-lg border border-[var(--color-border)] bg-transparent px-3 sm:px-4 py-1.5 sm:py-2.5 text-xs sm:text-sm text-[var(--color-text-secondary)] transition-colors hover:bg-[var(--color-bg-elevated)]"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="flex-1 rounded-lg bg-[var(--color-accent)] px-3 sm:px-4 py-1.5 sm:py-2.5 text-xs sm:text-sm font-semibold text-white shadow-[0_10px_24px_rgba(79,142,247,0.18)] transition-all duration-200 hover:bg-[#5d95f7] active:scale-[0.99] disabled:opacity-70 disabled:cursor-not-allowed"
                  >
